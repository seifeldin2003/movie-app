...
class HomeTab extends StatefulWidget {
  ...
  /// Called with a genre when a row's "See More" is tapped.
  ///
  /// This tab does not navigate — [LayoutScreen] does, by switching to Browse
  /// and selecting the genre. Same shape as `AppBottomNavBar.onTap`: the child
  /// reports, the shell decides.
  ///
  /// ⚠️ Optional so a widget test can still pump a bare `HomeTab()`.
              ...
              children: [
                ...
                for (final section in state.sections) ...[
                  GenreSection(
                    ...
                    // `section.title` is the genre string itself — HomeBloc
                    // builds each row's title straight from `AppGenres.homeRows`
                    // — so it matches a Browse chip with no mapping in between.
                    onSeeMore: onSeeMore == null
                        ? null
                        : () => onSeeMore!(section.title),
                  ),
                  SizedBox(height: 24.h),
                ],
